...
import cashFlowData from '@/data/cash_flow_allocation.json';
...
export default function CashFlowAllocation(props?: Partial<MarketSentimentGaugeProps>) {
  // Compute defaults from JSON data
  const allocation = Array.isArray((cashFlowData as any)?.cash_flow_allocation)
    ? (cashFlowData as any).cash_flow_allocation as Array<{ status: string; count: number }>
    : [];

  const increaseCount = allocation.find(a => a.status === 'INCREASE')?.count ?? 0;
  const decreaseCount = allocation.find(a => a.status === 'DECREASE')?.count ?? 0;
  const noChangeCount = allocation.find(a => a.status === 'NOCHANGE')?.count ?? 0;
  const totalCount = increaseCount + decreaseCount + noChangeCount;

  const computedIndex = totalCount > 0 ? Math.round((increaseCount / totalCount) * 100) : 0;

  const bandCount = 5;
  const percentPerBand = 100 / bandCount;
  const bandIndexFor = (p: number) => Math.floor(clamp(p, 0, 99.99) / percentPerBand);
  const computedResult = (SENTIMENT_BANDS[bandIndexFor(computedIndex)]?.label) ?? 'Trung tính';

  const displayIndex = props?.index ?? computedIndex;
  const displayResult = props?.result ?? computedResult;
  const width = 300;
  const height = 150;
  const cx = width / 2;
  const cy = height - 20;
  ...
  const activeBandIndex = Math.floor(clamp(displayIndex, 0, 99.99) / percentPerBand);

  // Calculate pointer angle based on index
  const pointerPercent = clamp(displayIndex, 0, 100);
